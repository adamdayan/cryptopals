...
def decrypt(target_bytes, key):
    ...
    decrypted_string = decrypted_target_bytes.decode("utf-8", errors="ignore")
...
def crack_single_byte_cypher(target_hex, sample_text_path):
    with open(sample_text_path, "r") as t:
        sample_text = t.read()

    reference_frequency = compute_letter_freq(sample_text)

    most_likely_key = compute_likely_key(target_hex, reference_frequency)
...
if __name__=="__main__":     
    ...
    crack_single_byte_cypher(intercept, "sample_text.txt")
